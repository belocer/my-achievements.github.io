...
$select_roller = "SELECT id_article, product_name, price, badge, path_file1 FROM dbelotserkovets_product WHERE category_name = 'roller' ORDER BY random() LIMIT 9;";
...
?>
	...
	<body>
		<?php require_once 'php/header.php'; ?>
			...
			<section>
				<div id="section">
					<span class="category">РОЛИКОВЫЕ КОНЬКИ</span>
					<br>
...
/*$select_snoubord = "SELECT product_name, price, badge, path_file1 FROM product WHERE category_name = 'snoubord' LIMIT 9;";*/
$select_roller1 = "SELECT id_article, product_name, price, badge, path_file1 FROM dbelotserkovets_product WHERE category_name = 'roller' ORDER BY random() LIMIT 4;";
$result_select_roller1 = pg_query($dbconn, "$select_roller1") or die('Ошибка запроса поиска записи: ' . pg_last_error());
$array_bd_roller_desc = pg_fetch_all($result_select_roller1);
foreach($array_bd_roller_desc as $key => $value){
	$v = substr($value["path_file1"], 1);
	if($value["badge"]=="new"){
			$badge = " newWrapper";
			$badge_img = '<div class="new"><img src="image/newgreen.png" alt="new"></div>';
...
echo '</div>';
		...
		// Очистка результатов
	pg_free_result($result_select_roller1);
