...
/*$select_snoubord = "SELECT product_name, price, badge, path_file1 FROM product WHERE category_name = 'scooter' LIMIT 9;";*/
$select_scooter = "SELECT id_article, product_name, price, badge, path_file1 FROM dbelotserkovets_product WHERE category_name = 'scooter' ORDER BY random() LIMIT 9;";
...
<!DOCTYPE html>
	...
	<body>
		<?php require_once 'php/header.php'; ?>
			...
			<section>
				<div id="section">
					<span class="category">САМОКАТЫ</span>
					<br>
...
$select_scooter1 = "SELECT id_article, product_name, price, badge, path_file1 FROM dbelotserkovets_product WHERE category_name = 'scooter' ORDER BY random() LIMIT 4;";
$result_select_scooter1 = pg_query($dbconn, "$select_scooter1") or die('Ошибка запроса поиска записи: ' . pg_last_error());
$array_bd_scooter_desc = pg_fetch_all($result_select_scooter1);
foreach($array_bd_scooter_desc as $key => $value){
	$v = substr($value["path_file1"], 1);
	if($value["badge"]=="new"){
			$badge = " newWrapper";
			$badge_img = '<div class="new"><img src="image/newgreen.png" alt="new"></div>';
...
echo '</div>';
		...
		// Очистка результатов
	pg_free_result($result_select_scooter1);
